...
class Octal
  attr_reader :octal_arr, :string

  def initialize(str)
    @octal_arr = str.to_i.digits
    @string = str
  end

  def to_decimal
    return 0 unless is_valid?(string)

    octal_arr.map.with_index do |num, index|
      num * (8 ** index)
    end.sum
  end
  ...
  def is_valid?(str)
    Integer(str, exception: false) && 
      octal_arr.all? { |int| ![8, 9].include?(int) }
  end
end
